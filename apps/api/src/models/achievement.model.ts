...
const achievementDefinitionSchema = new Schema<IAchievementDefinition>({
    ...
    category: {
        type: String,
        enum: ["gamesPlayed", "goals", "assists", "redCards", "passesMade", "passesSuccess", "manOfTheMatch", "hattricks", "pokers", "cleanSheets", "saves", "minutesPlayed", "pass_accuracy", "shot_accuracy"],
        required: true
    },
    scope: { type: String, enum: ["cumulative", "match"], required: true },
    type: { type: String, enum: ["official", "friendly"], required: true },

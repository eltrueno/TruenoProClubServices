...
<template>
  <div class="min-h-[60vh] flex items-center justify-center p-2 sm:p-4">
    <div class="card w-full max-w-sm bg-base-200 shadow-xl border border-base-300">
      <div class="card-body items-center text-center p-6 sm:p-8">
        ...
        <!-- No Logueado -->
        <div v-else-if="!isLoggedIn" class="w-full flex flex-col items-center gap-5">
           <div class="relative">
             <div class="absolute inset-0 bg-primary/20 blur-3xl rounded-full"></div>
             <div class="relative bg-base-300 p-4 rounded-2xl">
           ...
           <div class="space-y-1.5">
             <h2 class="text-2xl font-black tracking-tight uppercase">Mi Cuenta</h2>
             <p class="text-sm opacity-60 leading-relaxed">Inicia sesión para acceder a tu perfil y poder vincular tus cuentas, gestionar ajustes y obtener recompensas.</p>
           </div>

           <div class="card-actions w-full flex-col gap-3 mt-1">
             <div class="flex items-center justify-center gap-2 cursor-pointer group select-none w-full" @click="acceptedPrivacy = !acceptedPrivacy">
             ...
             <div :class="['w-full', { 'tooltip tooltip-bottom tooltip-warning': !acceptedPrivacy }]" data-tip="Debes aceptar la política de privacidad">
               <button 
                 ...
                 class="btn bg-[#9146FF] hover:bg-[#772ce8] border-none text-white btn-block gap-2 transition-all duration-300 active:scale-[0.98] font-black tracking-wide group text-sm disabled:bg-[#9146FF]/30 disabled:text-white/30 disabled:cursor-not-allowed"
               >
   ...
   <dialog ref="privacyModal" id="privacy_modal" class="modal modal-bottom sm:modal-middle">
     <div class="modal-box bg-base-200 border border-base-300 shadow-2xl max-w-2xl text-left p-0 flex flex-col max-h-[85vh]">
      ...
      <div class="modal-action m-0 px-6 py-3 border-t border-base-300 shrink-0">
        <form method="dialog">

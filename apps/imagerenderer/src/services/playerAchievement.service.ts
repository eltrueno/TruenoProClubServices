...
const getAchievementImage = async (playerId: string, type: string, reached: number) => {
    ...
    const filepath = path.join(dir, `playerachievement_${playerId}_${type}_${reached}.jpeg`);
    if (!fs.existsSync(filepath)) {
        console.log("Image not cached. Capturing...")
        const filename = `playerachievement_${playerId}_${type}_${reached}`
        const img = await captureImage(`${BASE_URL}/imagegenerator/playerachievement?id=${encodeURIComponent(playerId)}&type=${type}&reached=${reached}`, filename, "#to_image", 960, 1080)
